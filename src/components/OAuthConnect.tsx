...
export const OAuthConnect: React.FC<OAuthConnectProps> = ({
  ...
  // Load saved credentials from localStorage - check BOTH formats for compatibility
  useEffect(() => {
    // First try legacy format (oauth_googledrive_client_id)
    let savedId = localStorage.getItem(`oauth_${provider}_client_id`);
    let savedSecret = localStorage.getItem(`oauth_${provider}_client_secret`);

    // If not found, try the new format from settings panel (aeroftp_oauth_settings)
    if (!savedId || !savedSecret) {
      try {
        const oauthSettings = JSON.parse(localStorage.getItem('aeroftp_oauth_settings') || '{}');
        const providerSettings = oauthSettings[provider];
        if (providerSettings?.clientId && !savedId) savedId = providerSettings.clientId;
        if (providerSettings?.clientSecret && !savedSecret) savedSecret = providerSettings.clientSecret;
      } catch (e) {
        console.warn('[OAuthConnect] Failed to parse oauth settings:', e);
      }
    }
